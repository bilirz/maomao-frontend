...
onMounted(async () => {
  ...
  player.on('timeupdate', () => {
    ...
    // 获取与当前时间匹配的弹幕
    const matchedDanmakus = danmakus.value.filter(
      (danmaku) => Math.abs(currentTime - danmaku.video_time) < 0.5
    )
    ...
    activeDanmakus.value = activeDanmakus.value.filter((danmaku) => {
      ...
      const isExpired =
        currentTime - (danmaku.displayStartTime || 0) > displayTime
      if (isExpired && danmaku.type === 'scroll') {
        occupiedRows.value = occupiedRows.value.filter(
          (row) => row !== danmaku.position / 25
        )
      }
      return !isExpired
    })
...
const fetchDanmakus = async () => {
  try {
    const response = await axios.get(
      `${apiUrl.value}/api/danmaku/${props.videoInfo.aid}`
    )
    danmakus.value = response.data.danmakus
  } catch (error) {
    console.error('获取danmakus时出错:', error)
  }
}
