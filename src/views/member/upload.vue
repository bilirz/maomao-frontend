<template>
  <div>
    ...
    <div v-if="showUploader">
      <mmCard title="投稿一个视频">
        ...
        >
          ...
          <template #tip>
            <div class="el-upload__tip">文件大小不能超过500M。</div>
          </template>
        </el-upload>
        <el-progress
          v-if="isIndeterminateLoading"
          :percentage="uploadProgress"
        />
        <div v-if="uploadProgress === 100 && isIndeterminateLoading">
          正在加载...
        </div>
      </mmCard>
    </div>
    ...
    <!-- 视频信息填写 -->
    <div v-else-if="!showSuccessResult">
      <mmCard title="填写视频信息">
        <!-- 视频信息表单 -->
        <el-form ref="VideoForm" :model="formData" label-position="top">
          ...
          <!-- 标签输入 -->
          <el-form-item
            :label="`视频标签 (${formData.tags.length}/12)`"
            prop="tags"
            :rules="[tagsRule]"
          >
            <div
  ...
  try {
    for (let i = 0; i < chunks.length; i++) {
      ...
      const response = await axios.post(
        `${apiUrl.value}/api/upload/video/chunk`,
        formData,
        {
    ...
    const mergeResponse = await axios.post(
      `${apiUrl.value}/api/upload/video/merge`,
      {
